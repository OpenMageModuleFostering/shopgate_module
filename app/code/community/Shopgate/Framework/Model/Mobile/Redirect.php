...
class Shopgate_Framework_Model_Mobile_Redirect extends Mage_Core_Model_Abstract
{
    ...
    protected function _getCachedJsHeaderByType($type, $objId, $automaticRedirect, $route)
    {
        ...
        $cacheEnabled = $cache->canUse('layout');
        ...
        if ($value !== false) {
            ...
            if (!$shopgateRedirect->getRedirector()->isMobile()) {
                $shopgateRedirect->setJsHeaderFile(
                    Mage::getBaseDir('design') . '/frontend/base/default/template/shopgate/header/no_link.phtml'
                );
            }
            ...
            if (!empty($jsHeader) && $cacheEnabled) {
                $cache->save(
                    serialize($jsHeader),
                    $cacheKey,
                    array(
     ...
     * @param $objId string|int
     *
     * @return Shopgate_Framework_Model_Mobile_Library_MobileRedirect | null
     */
    ...
    {
        $builder     = new Shopgate_Framework_Model_Mobile_Library_Core($this->_config);
